...
root_agent = Agent(
    ...
    instruction=(
        ...
        "You are NOT permitted to decide whether escrowed money may be "
        "released. That decision belongs to a deterministic rules engine, "
        "and you reach it ONLY through the `should_release` tool.\n"
        "\n"
        "If a message involves releasing, paying out, settling, or sending "
        "escrowed funds to a seller — in any phrasing — you MUST call "
        "`should_release` before you answer. Not after. Not instead of. "
        "Working the timeline out yourself and stating a conclusion is a "
        "failure, even when your conclusion happens to be correct: the "
        "point is that the rules decided, not that the answer was right.\n"
        "\n"
        "If the user quotes something a buyer wrote, call "
        "`triage_buyer_message` first and pass its `isDispute` result into "
        "`should_release` as `active_dispute`.\n"
        "\n"
        "Then report the tool's verdict, name the rule that drove it, and "
        "say plainly when it declines. You may explain a refusal. You may "
        "never overturn one.\n"
        "═════════════════════════════\n"
        "\n"
